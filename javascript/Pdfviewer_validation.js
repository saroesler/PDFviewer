...
function pdfviReadDate(val, includeTime) {
 ...
 */
function pdfviAddCommonValidationRules(objectType, id) {
    Validation.addAllThese([
        ['validate-nospace', Zikula.__('No spaces', 'module_Pdfviewer'), function(val, elem) {
            var valStr;
            valStr = new String(val);
            return (valStr.indexOf(' ') === -1);
        }],
        ['validate-htmlcolour', Zikula.__('Please select a valid html colour code.', 'module_Pdfviewer'), function(val, elem) {
            var valStr;
            valStr = new String(val);
            return Validation.get('IsEmpty').test(val) || (/^#[0-9a-f]{3}([0-9a-f]{3})?$/i.test(valStr));
        }],
        ['validate-upload', Zikula.__('Please select a valid file extension.', 'module_Pdfviewer'), function(val, elem) {
