 ...
 *
 * This is the base repository class for document entities.
 */
class Pdfviewer_Entity_Repository_Base_Document extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new Pdfviewer_Util_ListEntries($serviceManager);
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('document', 'workflowState');
            }
    
            // initialise Imagine preset manager instances
            $imageHelper = new Pdfviewer_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'document';
            $parameters[$objectType . 'ThumbManagerDatei'] = $imageHelper->getManager($objectType, 'datei', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use seperate preset for images in related items
                $parameters['relationThumbPreset'] = $imageHelper->getPreset('', '', 'Pdfviewer_relateditem', $context, $args);
            }
        }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Pdfviewer_Entity_Document', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pdfviewer_Entity_Document', 'tbl')
     ...
     *
     * @return array|Pdfviewer_Entity_Document retrieved data array or Pdfviewer_Entity_Document instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Pdfviewer_Entity_Document instances
     */
    ...
    protected function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryFiltersPerRegistry = ModUtil::apiFunc('Pdfviewer', 'category', 'buildFilterClauses', array('ot' => 'document', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters)
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'Pdfviewer') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved documents only
            $onlineStates = array('approved');
            $onlyOwn = (int) FormUtil::getPassedValue('own', 0, 'GETPOST');
            if ($onlyOwn == 1) {
                // allow the owner to see his deferred documents
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.datei = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.begindate = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numDocuments';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pdfviewer_Entity_Document', 'tbl');
     ...
     * @param int    $excludeId  Id of documents to exclude (optional).
     *
     * @return boolean result of this check, true if the given document does not already exist
     */
    ...
    protected function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        //$fu = new FilterUtil('Pdfviewer', $this);
