...
<div class="z-formrow">
    <label for="Pdfviewer_objecttype">{gt text='Object type'}:</label>
    <select id="Pdfviewer_objecttype" name="objecttype" size="1">
        <option value="document"{if $objectType eq 'document'} selected="selected"{/if}>{gt text='Documents'}</option>
    </select>
...
{if $properties ne null && is_array($properties)}
    ...
    {foreach item='propertyName' from=$properties}
        <div class="z-formrow">
            ...
            &nbsp;
            {selector_category name="`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='Pdfviewer' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
...
<div class="z-formrow">
    <label for="Pdfviewer_sorting">{gt text='Sorting'}:</label>
    <select id="Pdfviewer_sorting" name="sorting">
...
<div class="z-formrow">
    <label for="Pdfviewer_amount">{gt text='Amount'}:</label>
    <input type="text" id="Pdfviewer_amount" name="amount" size="10" value="{$amount|default:"5"}" />
</div>

<div class="z-formrow">
    <label for="Pdfviewer_template">{gt text='Template'}:</label>
    <select id="Pdfviewer_template" name="template">
...
<div id="customtemplatearea" class="z-formrow z-hide">
    <label for="Pdfviewer_customtemplate">{gt text='Custom template'}:</label>
    <input type="text" id="Pdfviewer__customtemplate" name="customtemplate" size="40" maxlength="80" value="{$customTemplate|default:''}" />
    <div class="z-sub z-formnote">{gt text='Example'}: <em>itemlist_{objecttype}_display.tpl</em></div>
</div>

<div class="z-formrow z-hide">
    <label for="Pdfviewer_filter">{gt text='Filter (expert option)'}:</label>
    <input type="text" id="Pdfviewer_filter" name="filter" size="40" value="{$filterValue|default:''}" />
